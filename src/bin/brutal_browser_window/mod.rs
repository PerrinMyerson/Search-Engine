...
mod native {
    ...
    fn browser_window_hover_status_text(
        ...
        let status = match hit {
            BrowserAppWindowHit::BackButton => browser_window_history_target(app, true)?
                .map(|target| target.status)
                .unwrap_or_else(|| "Back unavailable".to_owned()),
            BrowserAppWindowHit::ForwardButton => browser_window_history_target(app, false)?
                .map(|target| target.status)
    ...
    struct BrowserWindowHistoryTarget {
        source: String,
        status: String,
    }
    ...
    fn browser_window_history_target(
        app: &BrowserApp,
        backwards: bool,
    ) -> Result<Option<BrowserWindowHistoryTarget>> {
        ...
        Ok(Some(BrowserWindowHistoryTarget {
    ...
    mod tests {
        ...
        #[tokio::test]
        async fn browser_window_command_click_history_buttons_open_tabs() {
            let dir = tempfile::tempdir().unwrap();
            let first = dir.path().join("first.html");
            let second = dir.path().join("second.html");
            std::fs::write(
            ...
            let background = handle_browser_window_left_click(
                &mut app,
                &mut mode,
                0,
                0,
            ...
            assert!(background.dirty);
            assert!(!background.close);
            assert_eq!(mode, BrowserWindowMode::Page);
            assert_eq!(app.tab_count(), 2);
            assert_eq!(app.active_tab(), 0);
            assert_eq!(
            ...
            app.apply_action(BrowserAppAction::SwitchTab(0))
                ...
                .unwrap();
            app.apply_action(BrowserAppAction::Back).await.unwrap();
            let foreground = handle_browser_window_left_click(
                &mut app,
                &mut mode,
                0,
                0,
            ...
            assert!(foreground.dirty);
            assert!(!foreground.close);
            assert_eq!(app.tab_count(), 3);
            assert_eq!(app.active_tab(), 2);
            assert_eq!(
                app.active_session().unwrap().current().unwrap().title,
        ...
        #[tokio::test]
        async fn browser_window_middle_click_empty_page_dismisses_prompt() {
            let mut app = BrowserApp::open(
                "bench/browser-fixtures/static-text.html",
                BrowserAppOptions {
                    render: BrowserRenderOptions {
