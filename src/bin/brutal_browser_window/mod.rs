...
mod native {
    ...
    mod tests {
        ...
        #[tokio::test]
        async fn browser_window_page_keys_noop_when_text_control_focused() {
            let dir = tempfile::tempdir().unwrap();
            let path = dir.path().join("focused-page-keys.html");
            std::fs::write(
                &path,
                r#"<html><head><title>Focused Page Keys</title></head><body><form><input name="q" value="filled"></form><p>line 1</p><p>line 2</p><p>line 3</p><p>line 4</p><p>line 5</p></body></html>"#,
            )
            .unwrap();
            let mut app = BrowserApp::open(
                path.to_str().unwrap(),
