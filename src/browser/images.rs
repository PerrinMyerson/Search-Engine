...
fn decode_image_bytes_with_svg_base(
    ...
    match image_type.as_str() {
        ...
        | "image/jpe" | "image/jfif" | "image/pjpeg" | "image/x-pjpeg" | "image/x-jpeg"
            ...
            match tag.kind {
                TagKind::Opening => {
                    ...
                    match tag.name.as_str() {
                        "svg" => {
                            let attrs = svg_attrs_with_style_rules("svg", attrs, &style_rules);
                            width = attrs
                                .get("width")
                        ...
                        "symbol" => {
                            let attrs = svg_attrs_with_style_rules("symbol", attrs, &style_rules);
                            if !tag.self_closing {
                        ...
                        "rect" => {
                            let attrs = svg_attrs_with_style_rules("rect", attrs, &style_rules);
                        ...
                        "circle" => {
                            let attrs = svg_attrs_with_style_rules("circle", attrs, &style_rules);
                            if let Some(transform) = svg_child_transform(&transform_stack, &attrs) {
                                let shape = SvgShapeAttrs {
                                    attrs: svg_shape_attrs_with_inherited_paint(
                                        &paint_stack,
                        ...
                        "ellipse" => {
                            let attrs = svg_attrs_with_style_rules("ellipse", attrs, &style_rules);
                            if let Some(transform) = svg_child_transform(&transform_stack, &attrs) {
                                let shape = SvgShapeAttrs {
                                    attrs: svg_shape_attrs_with_inherited_paint(
                                        &paint_stack,
                        ...
                        "polygon" => {
                            let attrs = svg_attrs_with_style_rules("polygon", attrs, &style_rules);
                            if let Some(transform) = svg_child_transform(&transform_stack, &attrs) {
                                let shape = SvgShapeAttrs {
                                    attrs: svg_shape_attrs_with_inherited_paint(
                                        &paint_stack,
                        ...
                        "polyline" => {
                            let attrs = svg_attrs_with_style_rules("polyline", attrs, &style_rules);
                            if let Some(transform) = svg_child_transform(&transform_stack, &attrs) {
                                let shape = SvgShapeAttrs {
                                    attrs: svg_shape_attrs_with_inherited_paint(
                                        &paint_stack,
                        ...
                        "image" => {
                            let attrs = svg_attrs_with_style_rules("image", attrs, &style_rules);
                            if let Some(transform) = svg_child_transform(&transform_stack, &attrs) {
                        ...
                        "use" => {
                            let attrs = svg_attrs_with_style_rules("use", attrs, &style_rules);
                        ...
                        "style" => {
                            if let Some(style_text) = svg_style_text_after_tag(bytes, tag_end) {
                                svg_merge_style_rules(&mut style_rules, style_text);
                            }
                        }
                        _ => {}
                    }
...
pub(super) fn decode_simple_png(bytes: &[u8]) -> Option<DecodedImage> {
    ...
    let (pixels, rgb_pixels) = match interlace_method {
...
fn decode_png_scanlines(
    ...
    png_decoded_pixels_complete(pixel_count, pixels, rgb_pixels)
}

fn decode_adam7_png_scanlines(
    width: usize,
    height: usize,
    ...
    let full_row_bytes = width.checked_mul(bytes_per_pixel)?;
    let mut full_rows = vec![0u8; full_row_bytes.checked_mul(height)?];
    let mut offset = 0usize;

    for (start_x, start_y, step_x, step_y) in ADAM7_PASSES {
        let pass_width = adam7_pass_size(width, start_x, step_x);
        let pass_height = adam7_pass_size(height, start_y, step_y);
        if pass_width == 0 || pass_height == 0 {
            continue;
        }
        let row_bytes = pass_width.checked_mul(bytes_per_pixel)?;
        let mut previous = vec![0u8; row_bytes];
        let mut current = vec![0u8; row_bytes];
        for pass_y in 0..pass_height {
            ...
            for pass_x in 0..pass_width {
                let dest_x = start_x.checked_add(pass_x.checked_mul(step_x)?)?;
                let dest_y = start_y.checked_add(pass_y.checked_mul(step_y)?)?;
                if dest_x >= width || dest_y >= height {
                    continue;
                }
                let source_offset = pass_x.checked_mul(bytes_per_pixel)?;
                let dest_offset = dest_y
                    .checked_mul(full_row_bytes)?
                    .checked_add(dest_x.checked_mul(bytes_per_pixel)?)?;
                full_rows
                    .get_mut(dest_offset..dest_offset.checked_add(bytes_per_pixel)?)?
                    .copy_from_slice(current.get(source_offset..source_offset + bytes_per_pixel)?);
            }
            previous.copy_from_slice(&current);
        }
    }
...
fn adam7_pass_size(size: usize, start: usize, step: usize) -> usize {
    if start >= size {
        0
    } else {
...
fn push_png_grayscale_pixels(
    ...
    match color_type {
        0 => {
            let transparent_gray = png_grayscale_transparent_value(transparency, bit_depth);
            for &gray in row {
        ...
        2 => {
            let transparent_rgb = png_rgb_transparent_color(transparency, bit_depth);
...
fn png_grayscale_transparent_value(transparency: &[u8], bit_depth: u8) -> Option<u8> {
    let value = read_png_u16(transparency.get(0..2)?)?;
    png_transparent_sample_to_u8(value, bit_depth)
}

fn png_rgb_transparent_color(transparency: &[u8], bit_depth: u8) -> Option<[u8; 3]> {
    Some([
        png_transparent_sample_to_u8(read_png_u16(transparency.get(0..2)?)?, bit_depth)?,
        png_transparent_sample_to_u8(read_png_u16(transparency.get(2..4)?)?, bit_depth)?,
        png_transparent_sample_to_u8(read_png_u16(transparency.get(4..6)?)?, bit_depth)?,
    ])
}
...
const SVG_INHERITED_PAINT_ATTRS: &[&str] = &[
    "color",
    "fill",
    "fill-opacity",
    "opacity",
...
fn svg_child_paint_attrs(
    stack: &[HashMap<String, String>],
    attrs: &HashMap<String, String>,
) -> HashMap<String, String> {
    let mut inherited = stack.last().cloned().unwrap_or_default();
    svg_merge_inherited_paint_attrs(&mut inherited, attrs);
    inherited
}

fn svg_shape_attrs_with_inherited_paint(
    stack: &[HashMap<String, String>],
    attrs: HashMap<String, String>,
) -> HashMap<String, String> {
    let mut merged = stack.last().cloned().unwrap_or_default();
    svg_merge_shape_attrs(&mut merged, attrs);
    merged
}

fn svg_merge_inherited_paint_attrs(
    inherited: &mut HashMap<String, String>,
    attrs: &HashMap<String, String>,
) {
    for property in SVG_INHERITED_PAINT_ATTRS {
        if let Some(value) = attrs.get(*property) {
            inherited.insert((*property).to_owned(), value.clone());
        }
    }
    if let Some(style) = attrs.get("style") {
        svg_remove_style_overridden_paint_attrs(inherited, style);
        svg_merge_style_attr(inherited, style);
    }
}

fn svg_merge_shape_attrs(inherited: &mut HashMap<String, String>, attrs: HashMap<String, String>) {
    for (key, value) in attrs {
        if key == "style" {
            svg_remove_style_overridden_paint_attrs(inherited, &value);
            svg_merge_style_attr(inherited, &value);
        } else {
...
#[derive(Default)]
struct SvgStyleRules {
    tag_styles: HashMap<String, HashMap<String, String>>,
    class_styles: HashMap<String, HashMap<String, String>>,
    id_styles: HashMap<String, HashMap<String, String>>,
}

fn svg_attrs_with_style_rules(
    tag_name: &str,
    mut attrs: HashMap<String, String>,
    style_rules: &SvgStyleRules,
) -> HashMap<String, String> {
    let inline_style = attrs.get("style").cloned().unwrap_or_default();
    if let Some(style_attrs) = style_rules.tag_styles.get(tag_name) {
        svg_apply_style_attrs(&mut attrs, &inline_style, style_attrs);
    }
    if let Some(class_attr) = attrs.get("class").cloned() {
        for class_name in class_attr.split_ascii_whitespace() {
            if let Some(style_attrs) = style_rules.class_styles.get(class_name) {
                svg_apply_style_attrs(&mut attrs, &inline_style, style_attrs);
            }
        }
    }
    if let Some(id) = attrs.get("id").cloned()
        && let Some(style_attrs) = style_rules.id_styles.get(&id)
    {
        svg_apply_style_attrs(&mut attrs, &inline_style, style_attrs);
    }
    attrs
...
fn svg_apply_style_attrs(
    attrs: &mut HashMap<String, String>,
    inline_style: &str,
    style_attrs: &HashMap<String, String>,
) {
...
fn svg_merge_style_rules(style_rules: &mut SvgStyleRules, style: &str) {
    let mut cursor = 0usize;
    while let Some(open_offset) = style[cursor..].find('{') {
        let selector_start = cursor;
        let selector_end = cursor + open_offset;
        let body_start = selector_end + 1;
        let Some(close_offset) = style[body_start..].find('}') else {
            break;
        };
        let body_end = body_start + close_offset;
        let declarations = svg_class_style_declarations(&style[body_start..body_end]);
        if !declarations.is_empty() {
            for selector in style[selector_start..selector_end].split(',') {
                svg_merge_style_selector(style_rules, selector, &declarations);
            }
        }
        cursor = body_end + 1;
    }
...
fn svg_merge_style_selector(
    style_rules: &mut SvgStyleRules,
    selector: &str,
    declarations: &HashMap<String, String>,
) {
    let selector = svg_clean_style_selector(selector);
    ...
    if let Some(class_name) = selector.strip_prefix('.') {
        if svg_style_identifier_supported(class_name) {
            style_rules
                .class_styles
    ...
    if let Some(id) = selector.strip_prefix('#') {
        if svg_style_identifier_supported(id) {
            style_rules
                .id_styles
                .entry(id.to_owned())
                .or_default()
    ...
    if svg_style_identifier_supported(selector) {
        style_rules
            .tag_styles
...
fn svg_clean_style_selector(selector: &str) -> &str {
    selector
        .trim()
...
fn svg_style_identifier_supported(value: &str) -> bool {
    !value.is_empty()
        && value
            .chars()
...
fn svg_remove_style_overridden_paint_attrs(attrs: &mut HashMap<String, String>, style: &str) {
    for property in SVG_INHERITED_PAINT_ATTRS {
        if svg_style_declares_property(style, property) {
            attrs.remove(*property);
        }
    }
}

fn svg_style_declares_property(style: &str, property: &str) -> bool {
...
fn svg_merge_style_attr(attrs: &mut HashMap<String, String>, style: &str) {
    match attrs.get_mut("style") {
        ...
        _ => {
            attrs.insert("style".to_owned(), style.to_owned());
        }
    }
}
...
fn svg_shape_paint(attrs: &HashMap<String, String>, property: &str) -> Option<SvgPaint> {
    ...
    if svg_paint_is_current_color(value) {
        let rgb = svg_shape_current_color_rgb(attrs).unwrap_or([0, 0, 0]);
...
fn svg_shape_current_color_rgb(attrs: &HashMap<String, String>) -> Option<[u8; 3]> {
    let color = svg_shape_paint_value(attrs, "color")?;
    parse_svg_css_color_rgb(color)
}
...
pub(super) fn background_image_sizes_attr(element: &ElementData) -> Option<&str> {
    first_non_empty_attr(
        ...
        &[
            ...
            "data-bgsizes",
            "data-bg-set-sizes",
            "data-bg-srcset-sizes",
            "data-bg-src-set-sizes",
            "data-bgsrcset-sizes",
            "data-bgset-sizes",
            "data-bgsetsizes",
            "data-background-sizes",
            "data-backgroundsizes",
            "data-background-set-sizes",
            "data-background-srcset-sizes",
            "data-backgroundsrcsetsizes",
            "data-lazy-bg-sizes",
            "data-lazybgsizes",
            "data-lazy-bg-set-sizes",
            "data-lazy-bg-srcset-sizes",
            "data-lazy-bg-src-set-sizes",
            "data-lazybgsrcset-sizes",
            "data-lazy-bgset-sizes",
            "data-lazybgsetsizes",
            "data-lazy-background-sizes",
            "data-lazybackgroundsizes",
            "data-lazy-background-set-sizes",
...
fn picture_source_lazy_srcset(
    ...
    picture_source_attr(
        ...
        &[
            ...
            "data-zoomsrcset",
            "data-gallery-srcset",
            "data-gallery-src-set",
            "data-gallerysrcset",
            "data-product-srcset",
            "data-product-src-set",
            "data-productsrcset",
            "data-thumb-srcset",
            "data-thumb-src-set",
            "data-thumbsrcset",
            "data-thumbnail-srcset",
            "data-thumbnail-src-set",
            "data-thumbnailsrcset",
            "current-srcset",
            "current-src-set",
            "currentsrcset",
            "data-src",
