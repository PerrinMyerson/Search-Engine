...
async fn image_picture_source_media_alignment_skips_print_resource_for_rgb_attachment() {
    ...
    fs::write(
        ...
        r#"<html><body>
            ...
            <picture>
                ...
                <source media="screen and (min-width: 9999px)" type="image/gif" srcset="wide.gif 32w">
                <source media="screen and (max-width: 800px)" type="image/gif" srcset="screen.gif 32w">
    ...
    assert!(
        !initial_render
            .resources
            .iter()
            .any(|resource| resource.url == "wide.gif")
    );
    ...
    assert!(
        !report
            .fetches
            .iter()
            .any(|fetch| fetch.resource.url == "wide.gif")
    );
    assert!(
        !report
            .fetches
...
async fn image_external_stylesheet_background_resource_renders_visible_color() {
    ...
    assert_eq!(
        fs::canonicalize(&stylesheet_report.fetches[0].resource.resolved).unwrap(),
        fs::canonicalize(&stylesheet).unwrap()
    );
...
#[tokio::test]
async fn image_external_stylesheet_relative_background_url_resolves_from_stylesheet_base() {
    let dir = tempfile::tempdir().unwrap();
    let pages = dir.path().join("pages");
    let css_dir = dir.path().join("css").join("app");
    let image_dir = dir.path().join("css").join("img");
    fs::create_dir_all(&pages).unwrap();
    fs::create_dir_all(&css_dir).unwrap();
    fs::create_dir_all(&image_dir).unwrap();
    let page = pages.join("index.html");
    let stylesheet = css_dir.join("site.css");
    let hero = image_dir.join("hero-bg.gif");
    fs::write(&hero, tiny_test_gif_palette()).unwrap();
    fs::write(
        &stylesheet,
        ".hero { background-image: url('../img/hero-bg.gif'); background-repeat: no-repeat; min-height: 24px; }",
    )
    .unwrap();
    fs::write(
        &page,
        r#"<html><head>
            ...
            <section class="hero">Nested stylesheet background</section>
        </body></html>"#,
    )
    .unwrap();
    ...
    let hero_canonical = fs::canonicalize(&hero).unwrap();
    let fetch = image_report
        .fetches
        .iter()
        .find(|fetch| {
            fs::canonicalize(&fetch.resource.resolved)
                .is_ok_and(|resolved| resolved == hero_canonical)
        })
        .expect("stylesheet-relative background image should resolve from stylesheet directory");
    assert_eq!(fetch.resource.kind, "background_image");
    assert_eq!(fetch.resource.initiator, "css");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("image/gif"));
    ...
    assert!(render.display_list.iter().any(|command| {
        matches!(
            ...
            DisplayCommand::BackgroundImage {
                ...
                decoded_hash: Some(hash),
                ..
            } if fs::canonicalize(url).is_ok_and(|resolved| resolved == hero_canonical)
                && hash == &decoded_hash
        )
    }));
    ...
    assert!(
        raster
            ...
            .chunks_exact(4)
            .any(|pixel| { pixel[0] > 200 && pixel[1] < 40 && pixel[2] < 40 && pixel[3] == 255 }),
        "stylesheet-relative decoded background should paint visible red pixels"
    );
    assert!(
        raster
            .pixels
            .chunks_exact(4)
            .any(|pixel| { pixel[0] < 40 && pixel[1] > 150 && pixel[2] < 40 && pixel[3] == 255 }),
        "stylesheet-relative decoded background should paint visible green pixels"
    );
    assert!(
        raster
            .pixels
            .chunks_exact(4)
            .any(|pixel| { pixel[0] < 40 && pixel[1] < 40 && pixel[2] > 180 && pixel[3] == 255 }),
        "stylesheet-relative decoded background should paint visible blue pixels"
    );
}

#[tokio::test]
async fn stylesheet_background_redirect_sniffs_color_and_attaches() {
    ...
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            if first_line.contains(" /sheet-bg-final ") {
                let response = format!(
            ...
            let body = br#"<html><head>
                ...
                <p>After stylesheet background redirect</p>
            </body></html>"#;
            let response = format!(
    ...
    assert_eq!(fetch.resource.kind, "background_image");
    assert_eq!(fetch.resource.initiator, "css");
    assert_eq!(fetch.resource.url, "/sheet-bg-redirect");
    ...
    assert!(
        render
            .text
            .contains("Before stylesheet background redirect")
    );
    assert!(render.text.contains("After stylesheet background redirect"));
    let rendered_image = render
        .decoded_images
        .iter()
...
#[tokio::test]
async fn color_image_viewport_paint_parity_pack_reports_scrolled_color_pixels() {
    let png_bytes = tiny_test_png_rgb_with_sub_filter();
    let decoded = decode_simple_png(&png_bytes).unwrap();
    let expected_hash = decoded.pixel_hash();
    let expected_color_hash = decoded.color_pixel_hash().unwrap();
    ...
    let capped_page = dir.path().join("capped.html");
    let capped = dir.path().join("capped.png");
    for path in [&background, &inline, &picture, &offscreen, &capped] {
    ...
    fs::write(
        ...
        r#"<html><head>
            ...
            <p>Intro two</p>
            <p>Intro three</p>
            <section class="hero">Viewport color background</section>
            <img src="inline.png" alt="Viewport inline color" width="16" height="24">
            <picture>
                <source media="print" type="image/png" srcset="print.png 1x">
                <source media="screen" type="image/png" srcset="picture.png 1x">
                <img src="fallback.png" alt="Viewport picture color" width="16" height="24">
            </picture>
            <p>Spacer one</p>
            <p>Spacer two</p>
            <p>Spacer three</p>
            ...
            <p>Spacer five</p>
            <p>Spacer six</p>
            <img src="offscreen.png" alt="Offscreen color" width="16" height="24">
        </body></html>"#,
    )
    .unwrap();
    fs::write(
    ...
    session.navigate(&page.display().to_string()).await.unwrap();
    let image_report = session.render_current_with_images(1024).await.unwrap();
    assert_eq!(image_report.image_count, 4);
    assert_eq!(image_report.decoded, 4);
    assert_eq!(image_report.failed, 0);
    assert_eq!(image_report.skipped, 0);
    assert!(!image_report.fetches.iter().any(|fetch| {
        fetch.resource.url == "print.png" || fetch.resource.url == "fallback.png"
    }));
    ...
    let visible_urls = [
        ...
        inline.display().to_string(),
        picture.display().to_string(),
    ];
    let visible_bounds = visible_urls
        .iter()
        .map(|target_url| {
            render
                .display_list
        ...
        .collect::<Vec<_>>();
    let viewport_y = visible_bounds.iter().map(|(y, _)| *y).min().unwrap();
    assert!(viewport_y > 0);
    let viewport_bottom = visible_bounds
        .iter()
        .map(|(y, height)| y.saturating_add(*height))
        .max()
        .unwrap();
    let viewport_height = viewport_bottom.saturating_sub(viewport_y).max(1);
    let offscreen_y = render
        ...
        .find_map(|command| match command {
            DisplayCommand::Image {
                ...
                decoded_hash: Some(hash),
                ..
            } if url == &offscreen.display().to_string() && hash == &expected_hash => Some(*y),
            _ => None,
        })
        .unwrap();
    ...
    assert_eq!(raster_report.raster_viewport_y, Some(viewport_y));
    assert_eq!(raster_report.raster_viewport_height, Some(viewport_height));
    assert_eq!(raster_report.visible_decoded_image_count, 3);
    assert_eq!(raster_report.culled_decoded_image_count, 1);
    assert!(raster_report.non_background_pixels > 0);
    assert!(raster_report.color_pixels > 0);
    assert_eq!(
        raster_report.color_pixels,
        raster
            .pixels
            .chunks_exact(4)
            .filter(|pixel| {
...
#[tokio::test]
async fn image_background_bmp_resource_attaches_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let hero = dir.path().join("hero.bmp");
    fs::write(&hero, tiny_test_bmp_rgb()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before BMP background</p>
            <section data-bg-src="hero.bmp">BMP background hero</section>
            <p>After BMP background</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert_eq!(resource_fetch.resource.url, "hero.bmp");
    assert_eq!(resource_fetch.status, "fetched");
    assert_eq!(resource_fetch.content_type.as_deref(), Some("image/bmp"));
    assert_eq!(
        resource_fetch.image_decode_status.as_deref(),
        Some("decoded")
    );
    assert_eq!(resource_fetch.diagnostic.as_deref(), Some("image_decoded"));
    assert!(resource_fetch.decoded_hash.is_some());
    assert!(resource_fetch.decoded_color_hash.is_some());
    assert!(
        resource_fetch
    ...
    assert_eq!(fetch.resource.url, "hero.bmp");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("image/bmp"));
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert!(fetch.decoded_color_bytes.is_some_and(|bytes| bytes > 0));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before BMP background"));
    assert!(render.text.contains("After BMP background"));
    let rendered = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_srcset_resource_sizes_media_uses_viewport_for_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let selected = dir.path().join("selected.gif");
    ...
    fs::write(
        ...
        r#"<html><body>
            ...
            <img
                ...
                width="80"
                height="24"
                alt="Media sized foreground resource">
            <p>After media sizes image</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before media sizes image"));
    assert!(render.text.contains("After media sizes image"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_srcset_density_resource_selects_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let selected = dir.path().join("selected.gif");
    let retina = dir.path().join("retina.gif");
    fs::write(&selected, tiny_test_gif_palette()).unwrap();
    fs::write(&retina, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before density image</p>
            <img
                src="/assets/fallback.gif"
                srcset="selected.gif 1x, retina.gif 2x"
                width="80"
                height="24"
                alt="Density foreground resource">
            <p>After density image</p>
        </body></html>"#,
    )
    .unwrap();

    let selected_url = selected.display().to_string();
    let retina_url = retina.display().to_string();
    let mut resource_session = BrowserSession::new(BrowserRenderOptions {
        width: 64,
        ..BrowserRenderOptions::default()
    });
    ...
    assert!(
        !resource_report
            .resources
            .iter()
            .any(|fetch| fetch.resource.resolved == retina_url)
    );
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before density image"));
    assert!(render.text.contains("After density image"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_srcset_skips_unsupported_data_density_for_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let selected = dir.path().join("selected.gif");
    fs::write(&selected, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before density srcset</p>
            <img
                src="/assets/fallback.gif"
                srcset="data:image/avif;base64,AAAA 2x, selected.gif 1x"
                width="80"
                height="24"
                alt="Density foreground resource">
            <p>After density srcset</p>
        </body></html>"#,
    )
    .unwrap();

    let unsupported = "data:image/avif;base64,AAAA";
    let selected_url = selected.display().to_string();
    let mut resource_session = BrowserSession::new(BrowserRenderOptions {
        width: 64,
        ..BrowserRenderOptions::default()
    ...
    assert_eq!(resource_report.failed, 0);
    assert!(
        !resource_report.resources.iter().any(
            |fetch| fetch.resource.url == unsupported || fetch.resource.resolved == unsupported
        )
    );
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before density srcset"));
    assert!(render.text.contains("After density srcset"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_extensionless_generic_png_resource_sniffs_visible_rgb_candidate() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let server = tokio::spawn(async move {
        for _ in 0..2 {
            ...
            let (content_type, body) = if first_line.contains(" /cdn-image ") {
                ...
                (
                    "text/html",
                    br#"<html><body><p>Before sniffed PNG</p><img src="/cdn-image" alt="Sniffed PNG" width="16" height="24"><p>After sniffed PNG</p></body></html>"#.to_vec(),
                )
            };
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: {content_type}\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before sniffed PNG"));
    assert!(render.text.contains("After sniffed PNG"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == expected_hash)
    ...
    assert!(render.display_list.iter().any(|command| {
        matches!(
            ...
            DisplayCommand::Image {
                ...
                decoded_hash: Some(hash),
                ..
            } if url == &image_url && hash == &expected_hash
        )
    }));
...
#[tokio::test]
async fn image_text_plain_png_resource_sniffs_visible_rgb_candidate() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let server = tokio::spawn(async move {
        for _ in 0..2 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            let (content_type, body) = if first_line.contains(" /plain-image ") {
                ("text/plain", png_bytes.clone())
            } else {
                (
                    "text/html",
                    br#"<html><body><p>Before plain PNG</p><img src="/plain-image" alt="Plain PNG" width="16" height="24"><p>After plain PNG</p></body></html>"#.to_vec(),
                )
            };
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: {content_type}\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
    ...
    let image_url = format!("http://{addr}/plain-image");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 40,
        ..BrowserRenderOptions::default()
    });
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "/plain-image");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("text/plain"));
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert_eq!(fetch.decoded_width, Some(decoded.width));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before plain PNG"));
    assert!(render.text.contains("After plain PNG"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == expected_hash)
...
async fn image_local_jpeg_resource_decodes_visible_rgb_candidate() {
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before local JPEG</p>
            <img src="hero.jpg" alt="Local JPEG hero" width="80" height="24">
            <p>After local JPEG</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let resource_fetch = resource_report
        ...
        .iter()
        .find(|fetch| fetch.resource.resolved == hero_url)
        .expect("local JPEG resource is reported");
    assert_eq!(resource_fetch.resource.kind, "image");
    assert_eq!(resource_fetch.resource.initiator, "img");
    ...
    let fetch = report
        ...
        .iter()
        .find(|fetch| fetch.resource.resolved == hero_url)
        .expect("local JPEG render fetch is reported");
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before local JPEG"));
    assert!(render.text.contains("After local JPEG"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
        .expect("decoded JPEG image is attached to render output");
    assert_eq!(
        rendered_image.image.color_pixel_hash().as_deref(),
        Some(color_hash.as_str())
    );
...
#[tokio::test]
async fn image_extensionless_generic_svg_resource_sniffs_visible_rgb_candidate() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;

    let svg_bytes = br#"<?xml version="1.0"?><!-- CDN logo --><svg width="3" height="2" xmlns="http://www.w3.org/2000/svg"><rect width="1" height="2" fill="red"/><rect x="1" width="1" height="2" fill="lime"/><rect x="2" width="1" height="2" fill="blue"/></svg>"#.to_vec();
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        for _ in 0..2 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            let (content_type, body) = if first_line.contains(" /cdn-logo ") {
                ("application/octet-stream", svg_bytes.clone())
            } else {
                (
                    "text/html",
                    br#"<html><body><p>Before generic SVG</p><img src="/cdn-logo" alt="Generic SVG logo" width="30" height="20"><p>After generic SVG</p></body></html>"#.to_vec(),
                )
            };
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: {content_type}\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
    ...
    let image_url = format!("http://{addr}/cdn-logo");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "/cdn-logo");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(
        fetch.content_type.as_deref(),
        Some("application/octet-stream")
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before generic SVG"));
    assert!(render.text.contains("After generic SVG"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn redirected_extensionless_svg_without_content_type_sniffs_and_attaches_color() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;

    let svg_bytes = br#"<?xml version="1.0"?><svg width="3" height="2" xmlns="http://www.w3.org/2000/svg"><rect width="1" height="2" fill="red"/><rect x="1" width="1" height="2" fill="lime"/><rect x="2" width="1" height="2" fill="blue"/></svg>"#.to_vec();
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            if first_line.contains(" /svg-redirect ") {
                stream
                    .write_all(
                        b"HTTP/1.1 302 Found\r\nLocation: /svg-final-resource\r\nContent-Length: 0\r\nConnection: close\r\n\r\n",
                    )
                    .await
                    .unwrap();
                continue;
            ...
            if first_line.contains(" /svg-final-resource ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    svg_bytes.len()
                );
            ...
            assert!(first_line.contains(" /page.html "), "{first_line}");
            let body = br#"<html><body><p>Before redirected SVG</p><img src="/svg-redirect" alt="Redirected SVG" width="30" height="20"><p>After redirected SVG</p></body></html>"#;
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                body.len()
            );
    ...
    let page_url = format!("http://{addr}/page.html");
    let original_url = format!("http://{addr}/svg-redirect");
    let final_url = format!("http://{addr}/svg-final-resource");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "/svg-redirect");
    assert_eq!(fetch.resource.resolved, original_url);
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.source.as_deref(), Some(final_url.as_str()));
    assert_eq!(fetch.content_type, None);
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert_eq!(fetch.image_decode_error_kind, None);
    assert_eq!(fetch.image_byte_signature.as_deref(), Some("image/svg+xml"));
    assert_eq!(fetch.decoded_width, Some(3));
    assert_eq!(fetch.decoded_height, Some(2));
    assert_eq!(fetch.decoded_has_alpha, Some(false));
    assert!(fetch.decoded_hash.is_some());
    assert!(fetch.decoded_color_hash.is_some());
    assert_eq!(fetch.decoded_color_bytes, Some(18));
    let decoded_hash = fetch.decoded_hash.clone().unwrap();
    let color_hash = fetch.decoded_color_hash.clone().unwrap();

    let render = session.current().unwrap();
    assert!(render.text.contains("Before redirected SVG"));
    assert!(render.text.contains("After redirected SVG"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == original_url)
        .unwrap();
    assert_eq!(rendered_image.pixel_hash, decoded_hash);
    assert_eq!(
        rendered_image.image.color_pixel_hash().as_deref(),
        Some(color_hash.as_str())
    );
    assert_eq!(
        rendered_image.image.rgb_pixels.as_ref().map(Vec::len),
        fetch.decoded_color_bytes
    );
    assert!(render.display_list.iter().any(|command| {
        matches!(
            ...
            DisplayCommand::Image {
                ...
                decoded_hash: Some(hash),
                ..
            } if url == &original_url && hash == &decoded_hash
        )
    }));
...
async fn img_srcset_protocol_relative_redirect_sniffs_color_and_attaches() {
    ...
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let protocol_relative = format!("//{addr}/proto-redirect");
    let protocol_large = format!("//{addr}/proto-large");
    let server_protocol_relative = protocol_relative.clone();
    let server_protocol_large = protocol_large.clone();
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            if first_line.contains(" /proto-redirect ") {
                stream
                    .write_all(
                        b"HTTP/1.1 302 Found\r\nLocation: /proto-final-image\r\nContent-Length: 0\r\nConnection: close\r\n\r\n",
                    )
                    .await
                    .unwrap();
                continue;
            ...
            if first_line.contains(" /proto-final-image ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    png_bytes.len()
                );
            ...
            let body = format!(
                r#"<html><body>
                    <p>Before protocol srcset</p>
                    <img
                        src="/fallback.gif"
                        srcset="/dead.avif 40w, {server_protocol_relative} 80w, {server_protocol_large} 320w"
                        sizes="80px"
                        alt="Protocol srcset color"
                        width="80"
                        height="24">
                    <p>After protocol srcset</p>
                </body></html>"#
            );
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
    ...
    let page_url = format!("http://{addr}/page.html");
    let selected_url = format!("http://{addr}/proto-redirect");
    let final_url = format!("http://{addr}/proto-final-image");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before protocol srcset"));
    assert!(render.text.contains("After protocol srcset"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == selected_url)
...
#[tokio::test]
async fn img_srcset_media_sizes_redirect_selects_viewport_color_candidate() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            if first_line.contains(" /viewport-redirect ") {
                stream
                    .write_all(
                        b"HTTP/1.1 302 Found\r\nLocation: /viewport-final\r\nContent-Length: 0\r\nConnection: close\r\n\r\n",
                    )
                    .await
                    .unwrap();
                continue;
            ...
            if first_line.contains(" /viewport-final ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    png_bytes.len()
                );
            ...
            assert!(first_line.contains(" /page.html "), "{first_line}");
            let body = br#"<html><body>
                <p>Before viewport srcset</p>
                <img
                    src="/fallback.gif"
                    srcset="/viewport-redirect 80w, /viewport-large 320w"
                    sizes="(max-width: 400px) 80px, 320px"
                    alt="Viewport srcset color"
                    width="80"
                    height="24">
                <p>After viewport srcset</p>
            </body></html>"#;
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                body.len()
    ...
    let page_url = format!("http://{addr}/page.html");
    let selected_url = format!("http://{addr}/viewport-redirect");
    let final_url = format!("http://{addr}/viewport-final");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    assert!(
        !report
            .fetches
            .iter()
            .any(|fetch| fetch.resource.url == "/viewport-large"
                || fetch.resource.url == "/fallback.gif")
    );
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "/viewport-redirect");
    assert_eq!(fetch.resource.resolved, selected_url);
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.source.as_deref(), Some(final_url.as_str()));
    assert_eq!(fetch.content_type.as_deref(), Some("text/plain"));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before viewport srcset"));
    assert!(render.text.contains("After viewport srcset"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == selected_url)
...
#[tokio::test]
async fn image_picture_source_type_allows_typed_candidate_with_misleading_extension() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before typed picture mismatch</p>
            <picture>
                <source type="image/webp" srcset="hero.avif 80w">
                <img src="fallback.gif" alt="Typed WebP source" width="80" height="24">
            </picture>
            <p>After typed picture mismatch</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before typed picture mismatch"));
    assert!(render.text.contains("After typed picture mismatch"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_preload_imagesrcset_redirect_sniffs_color_and_attaches() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            if first_line.contains(" /preload-redirect ") {
                stream
                    .write_all(
                        b"HTTP/1.1 302 Found\r\nLocation: /preload-final\r\nContent-Length: 0\r\nConnection: close\r\n\r\n",
                    )
                    .await
                    .unwrap();
                continue;
            ...
            if first_line.contains(" /preload-final ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    png_bytes.len()
                );
            ...
            assert!(first_line.contains(" /page.html "), "{first_line}");
            let body = br#"<html><head>
                <link rel="preload" as="image" href="/fallback.gif" imagesrcset="/dead.avif 80w, /preload-redirect 160w, /preload-large 640w" imagesizes="160px">
            </head><body>
                <p>Before preload redirect</p>
                <img src="/preload-redirect" alt="Preloaded redirect" width="160" height="24">
                <p>After preload redirect</p>
            </body></html>"#;
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                body.len()
    ...
    let page_url = format!("http://{addr}/page.html");
    let selected_url = format!("http://{addr}/preload-redirect");
    let final_url = format!("http://{addr}/preload-final");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    assert!(
        !report
            ...
            .iter()
            .any(|fetch| fetch.resource.url == "/dead.avif"
                || fetch.resource.url == "/preload-large"
                || fetch.resource.url == "/fallback.gif")
    );
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "link");
    assert_eq!(fetch.resource.url, "/preload-redirect");
    assert_eq!(fetch.resource.resolved, selected_url);
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.source.as_deref(), Some(final_url.as_str()));
    assert_eq!(fetch.content_type.as_deref(), Some("text/plain"));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before preload redirect"));
    assert!(render.text.contains("After preload redirect"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == selected_url)
...
#[tokio::test]
async fn image_lazy_srcset_auto_sizes_uses_width_for_visible_color_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("loading.gif");
    let selected = dir.path().join("selected.gif");
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before auto lazy srcset</p>
            <img
                src="loading.gif"
                data-srcset="oversized.gif 640w, selected.gif 80w"
                sizes="auto"
                alt="Auto lazy source set"
                width="80"
                height="24">
            <p>After auto lazy srcset</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before auto lazy srcset"));
    assert!(render.text.contains("After auto lazy srcset"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_picture_typed_extensionless_alias_selects_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("placeholder.gif");
    let hero = dir.path().join("cdn-product");
    fs::write(&placeholder, tiny_test_gif_palette()).unwrap();
    fs::write(&hero, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before typed extensionless picture</p>
            <picture>
                <source
                    type="image/gif"
                    data-current-src="cdn-product">
                <img src="placeholder.gif" alt="Typed extensionless picture" width="80" height="24">
            </picture>
            <p>After typed extensionless picture</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert_eq!(resource_fetch.resource.kind, "image");
    assert_eq!(resource_fetch.resource.initiator, "source");
    assert_eq!(resource_fetch.resource.url, "cdn-product");
    assert_eq!(
        resource_fetch.resource.type_hint.as_deref(),
        Some("image/gif")
    );
    assert_eq!(resource_fetch.status, "fetched");
    assert_eq!(resource_fetch.content_type, None);
    assert_eq!(
        resource_fetch.image_decode_status.as_deref(),
        Some("decoded")
    );
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "cdn-product");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type, None);
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert!(fetch.decoded_color_bytes.is_some_and(|bytes| bytes > 0));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before typed extensionless picture"));
    assert!(render.text.contains("After typed extensionless picture"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_direct_lazy_alias_prefers_real_candidate_over_placeholder_visible_rgb() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("loading.gif");
    let hero = dir.path().join("hero.webp");
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before direct lazy alias</p>
            <img src="loading.gif" data-src="loading.gif" data-large="hero.webp" alt="Real lazy hero" width="80" height="24">
            <p>After direct lazy alias</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before direct lazy alias"));
    assert!(render.text.contains("After direct lazy alias"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_zoom_url_alias_resource_matches_render_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("loading.gif");
    let hero = dir.path().join("zoom.gif");
    fs::write(&placeholder, tiny_test_gif_palette()).unwrap();
    fs::write(&hero, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before zoom alias</p>
            <img src="loading.gif" data-zoom-image-url="zoom.gif" alt="Zoom hero" width="80" height="24">
            <p>After zoom alias</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert_eq!(resource_fetch.resource.kind, "image");
    assert_eq!(resource_fetch.resource.initiator, "img");
    assert_eq!(resource_fetch.resource.url, "zoom.gif");
    assert_eq!(resource_fetch.status, "fetched");
    assert_eq!(resource_fetch.content_type.as_deref(), Some("image/gif"));
    assert_eq!(
        resource_fetch.image_decode_status.as_deref(),
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "zoom.gif");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("image/gif"));
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert!(fetch.decoded_color_bytes.is_some_and(|bytes| bytes > 0));
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before zoom alias"));
    assert!(render.text.contains("After zoom alias"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_lazy_product_url_width_template_selects_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("loading.gif");
    let selected = dir.path().join("product_80.gif");
    let oversized = dir.path().join("product_320.gif");
    fs::write(&placeholder, tiny_test_gif_palette()).unwrap();
    fs::write(&selected, tiny_test_gif_palette()).unwrap();
    fs::write(&oversized, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before product URL template</p>
            <img
                src="loading.gif"
                data-product-image-url="product_{width}.gif"
                data-product-widths="[80, 320]"
                width="80"
                height="24"
                alt="Product URL width template">
            <p>After product URL template</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert!(
        !resource_report
            ...
            .any(|fetch| fetch.resource.resolved == placeholder_url
                || fetch.resource.resolved == oversized_url
                || fetch.resource.url == "loading.gif"
                || fetch.resource.url == "product_{width}.gif")
    );
    let resource_fetch = resource_report
        .resources
        .iter()
    ...
    assert_eq!(resource_fetch.resource.kind, "image");
    assert_eq!(resource_fetch.resource.initiator, "img");
    assert_eq!(resource_fetch.resource.url, "product_80.gif");
    assert_eq!(resource_fetch.status, "fetched");
    assert_eq!(resource_fetch.content_type.as_deref(), Some("image/gif"));
    assert_eq!(
        resource_fetch.image_decode_status.as_deref(),
    ...
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "product_80.gif");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("image/gif"));
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert!(fetch.decoded_color_bytes.is_some_and(|bytes| bytes > 0));
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before product URL template"));
    assert!(render.text.contains("After product URL template"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_lazy_width_template_skips_unsupported_candidate_for_visible_rgb_alias() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let selected = dir.path().join("hero.gif");
    fs::write(&selected, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before unsupported width template</p>
            <img
                src="/assets/loading.gif"
                data-src="dead_{width}.avif"
                data-widths="[80, 320]"
                data-original="hero.gif"
                width="80"
                height="24"
                alt="Fallback alias hero">
            <p>After unsupported width template</p>
        </body></html>"#,
    )
    .unwrap();

    let selected_url = selected.display().to_string();
    let unsupported_80 = dir.path().join("dead_80.avif").display().to_string();
    let unsupported_320 = dir.path().join("dead_320.avif").display().to_string();
    let mut resource_session = BrowserSession::new(BrowserRenderOptions::default());
    resource_session
        .navigate(&page.display().to_string())
        .await
    ...
    assert!(
        !resource_report
            .resources
            .iter()
            .any(|fetch| fetch.resource.resolved == unsupported_80
                || fetch.resource.resolved == unsupported_320
                || fetch.resource.url == "dead_{width}.avif"
                || fetch.resource.url.ends_with(".avif")
                || fetch.resource.url.contains("/assets/"))
    );
    let resource_fetch = resource_report
        .resources
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before unsupported width template"));
    assert!(render.text.contains("After unsupported width template"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_current_src_aliases_attach_visible_color_commands() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let hero = dir.path().join("hero.gif");
    let wide = dir.path().join("wide.gif");
    fs::write(&hero, tiny_test_gif_palette()).unwrap();
    fs::write(&wide, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before current aliases</p>
            <img data-currentSrc="hero.gif" alt="Current image" width="32" height="32">
            <picture>
                <source media="print" type="image/gif" data-currentSrcset="print-only.gif 32w">
                <source media="screen" type="image/gif" data-currentSrcset="wide.gif 32w">
                <img alt="Picture current image" width="32" height="32">
            </picture>
            <p>After current aliases</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert!(
        !report
            .fetches
            .iter()
            .any(|fetch| fetch.resource.url == "print-only.gif")
    );

    let mut decoded = Vec::new();
    for (file, url, kind, initiator) in [
        (&hero, "hero.gif", "image", "img"),
        (&wide, "wide.gif", "image", "img"),
    ] {
        let resolved = file.display().to_string();
        let fetch = report
            .fetches
        ...
        assert!(fetch.decoded_color_bytes.is_some_and(|bytes| bytes > 0));
        assert_eq!(fetch.render_attached, Some(true));
        assert_eq!(fetch.render_attachment_kind.as_deref(), Some("image"));
        assert_eq!(
            fetch.image_visibility_state.as_deref(),
            Some("render_attached")
        );
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before current aliases"));
    assert!(render.text.contains("After current aliases"));
    for (resolved, decoded_hash, color_hash) in decoded {
        let rendered_image = render
            .decoded_images
            .iter()
...
#[tokio::test]
async fn image_current_srcset_sizes_uses_viewport_for_resource_and_render_attachment() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let narrow = dir.path().join("narrow.gif");
    let selected = dir.path().join("selected.gif");
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before current sizes</p>
            <img
                data-currentSrcset="narrow.gif 80w, selected.gif 320w"
                sizes="50vw"
                alt="Current sizes image"
                width="80"
                height="32">
            <p>After current sizes</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert!(
        !resource_report.resources.iter().any(
            |fetch| fetch.resource.resolved == narrow_url || fetch.resource.url == "narrow.gif"
        )
    );
    let resource_fetch = resource_report
        .resources
    ...
    assert_eq!(resource_fetch.image_decode_error_kind, None);
    assert_eq!(
        resource_fetch.image_byte_signature.as_deref(),
        Some("image/gif")
    );
    assert!(resource_fetch.decoded_color_hash.is_some());
    assert!(
    ...
    let render = render_session.current().unwrap();
    assert!(render.text.contains("Before current sizes"));
    assert!(render.text.contains("After current sizes"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_current_src_redirect_keeps_original_url_and_color_attachment() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            let request = String::from_utf8_lossy(&buf[..n]);
            let first_line = request.lines().next().unwrap_or_default();
            if first_line.contains(" /current-redirect ") {
                stream
                    .write_all(
                        b"HTTP/1.1 302 Found\r\nLocation: /current-final\r\nContent-Length: 0\r\nConnection: close\r\n\r\n",
                    )
                    .await
                    .unwrap();
                continue;
            ...
            if first_line.contains(" /current-final ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    png_bytes.len()
                );
            ...
            assert!(first_line.contains(" /page.html "), "{first_line}");
            let body = br#"<html><body>
                <p>Before redirected current source</p>
                <img src="/placeholder.gif" data-current-src="/current-redirect" alt="Redirected current source" width="32" height="32">
                <p>After redirected current source</p>
            </body></html>"#;
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                body.len()
    ...
    let page_url = format!("http://{addr}/page.html");
    let original_url = format!("http://{addr}/current-redirect");
    let final_url = format!("http://{addr}/current-final");
    let placeholder_url = format!("http://{addr}/placeholder.gif");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 40,
        ..BrowserRenderOptions::default()
    });
    ...
    let fetch = report
        .fetches
        .iter()
        .find(|fetch| fetch.resource.resolved == original_url)
        .unwrap();
    assert_eq!(fetch.resource.kind, "image");
    assert_eq!(fetch.resource.initiator, "img");
    assert_eq!(fetch.resource.url, "/current-redirect");
    assert_eq!(fetch.resource.resolved, original_url);
    assert_eq!(fetch.source.as_deref(), Some(final_url.as_str()));
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("text/plain"));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before redirected current source"));
    assert!(render.text.contains("After redirected current source"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == original_url)
...
#[tokio::test]
async fn image_background_aliases_prefer_real_candidate_over_placeholder_visible_rgb() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("loading.gif");
    let hero = dir.path().join("hero.webp");
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before background placeholder alias</p>
            <section data-bg="loading.gif" data-background-image="url('hero.webp')">Background real alias</section>
            <p>After background placeholder alias</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before background placeholder alias"));
    assert!(render.text.contains("After background placeholder alias"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
async fn image_background_aliases_skip_unsupported_direct_candidate_for_visible_rgb() {
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before background unsupported alias</p>
            <section data-bg="dead.avif" data-background-image="url('hero.gif')">Background supported alias</section>
            <p>After background unsupported alias</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert!(
        !resource_report
            ...
            .iter()
            .any(|fetch| fetch.resource.url == "dead.avif"
                || fetch.resource.resolved.ends_with("/dead.avif"))
    );
    let resource_fetch = resource_report
        .resources
        .iter()
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before background unsupported alias"));
    assert!(render.text.contains("After background unsupported alias"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_background_imageset_skips_unsupported_data_candidate_for_visible_rgb() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let hero = dir.path().join("hero.gif");
    fs::write(&hero, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before data image-set background</p>
            <section data-lazy-background-image="image-set(url('data:image/avif;base64,AAAA') 2x, url('hero.gif') type('image/gif') 1x)">Data image-set background</section>
            <p>After data image-set background</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before data image-set background"));
    assert!(render.text.contains("After data image-set background"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_background_srcset_resource_matches_render_over_direct_placeholder() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let placeholder = dir.path().join("loading.gif");
    let selected = dir.path().join("selected-bg.gif");
    ...
    fs::write(
        &page,
        r#"<html><body>
            <p>Before background alignment</p>
            <section
                data-background-image="loading.gif"
                data-bg-srcset="oversized-bg.gif 640w, selected-bg.gif 80w"
                data-bg-srcset-sizes="80px">Background alignment</section>
            <p>After background alignment</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before background alignment"));
    assert!(render.text.contains("After background alignment"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn image_inline_style_imageset_background_selects_visible_rgb_candidate() {
    let dir = tempfile::tempdir().unwrap();
    let page = dir.path().join("page.html");
    let hero = dir.path().join("style-hero.gif");
    fs::write(&hero, tiny_test_gif_palette()).unwrap();
    fs::write(
        &page,
        r#"<html><body>
            <p>Before inline image-set background</p>
            <section style="background-image: image-set(url('data:image/avif;base64,AAAA') 2x, url('style-hero.gif') type('image/gif') 1x)">Inline image-set background</section>
            <p>After inline image-set background</p>
        </body></html>"#,
    )
    .unwrap();
    ...
    assert_eq!(resource_fetch.resource.kind, "background_image");
    assert_eq!(resource_fetch.resource.initiator, "section");
    assert_eq!(resource_fetch.resource.url, "style-hero.gif");
    assert_eq!(resource_fetch.status, "fetched");
    assert_eq!(resource_fetch.content_type.as_deref(), Some("image/gif"));
    assert_eq!(
        resource_fetch.image_decode_status.as_deref(),
    ...
    assert_eq!(fetch.resource.kind, "background_image");
    assert_eq!(fetch.resource.initiator, "section");
    assert_eq!(fetch.resource.url, "style-hero.gif");
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.content_type.as_deref(), Some("image/gif"));
    assert_eq!(fetch.image_decode_status.as_deref(), Some("decoded"));
    assert!(fetch.decoded_color_bytes.is_some_and(|bytes| bytes > 0));
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before inline image-set background"));
    assert!(render.text.contains("After inline image-set background"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.pixel_hash == decoded_hash)
...
#[tokio::test]
async fn css_imageset_background_redirect_sniffs_color_without_content_type() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            ...
            if first_line.contains(" /css-final-image ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    png_bytes.len()
                );
            ...
            assert!(first_line.contains(" /page.html "), "{first_line}");
            let body = br#"<html><body>
                <p>Before CSS image-set background</p>
                <section style="background-image: image-set(url(data:image/avif;base64,AAAA) type('image/avif') 1x, url(/css-redirect) type('image/png') 2x)">CSS background</section>
                <p>After CSS image-set background</p>
            </body></html>"#;
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                body.len()
    ...
    let page_url = format!("http://{addr}/page.html");
    let unsupported = "data:image/avif;base64,AAAA";
    let selected_url = format!("http://{addr}/css-redirect");
    let final_url = format!("http://{addr}/css-final-image");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    assert_eq!(fetch.resource.kind, "background_image");
    assert_eq!(fetch.resource.initiator, "section");
    assert_eq!(fetch.resource.url, "/css-redirect");
    assert_eq!(fetch.resource.resolved, selected_url);
    assert_eq!(fetch.status, "fetched");
    assert_eq!(fetch.source.as_deref(), Some(final_url.as_str()));
    assert_eq!(fetch.content_type, None);
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before CSS image-set background"));
    assert!(render.text.contains("After CSS image-set background"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == selected_url)
...
#[tokio::test]
async fn css_protocol_relative_background_redirect_keeps_color_evidence() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
    use tokio::net::TcpListener;
    ...
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let protocol_relative = format!("//{addr}/bg-redirect");
    let server_protocol_relative = protocol_relative.clone();
    let server = tokio::spawn(async move {
        for _ in 0..3 {
            let (mut stream, _) = listener.accept().await.unwrap();
            ...
            if first_line.contains(" /bg-final ") {
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
                    png_bytes.len()
                );
            ...
            let body = format!(
                r#"<html><body>
                    <p>Before protocol background</p>
                    <section style="background-image: linear-gradient(#fff, #eee), url({server_protocol_relative})">Protocol background</section>
                    <p>After protocol background</p>
                </body></html>"#
            );
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
    ...
    let page_url = format!("http://{addr}/page.html");
    let selected_url = format!("http://{addr}/bg-redirect");
    let final_url = format!("http://{addr}/bg-final");
    let mut session = BrowserSession::new(BrowserRenderOptions {
        width: 48,
        ..BrowserRenderOptions::default()
    });
    ...
    let render = session.current().unwrap();
    assert!(render.text.contains("Before protocol background"));
    assert!(render.text.contains("After protocol background"));
    let rendered_image = render
        .decoded_images
        .iter()
        .find(|image| image.url == selected_url)
