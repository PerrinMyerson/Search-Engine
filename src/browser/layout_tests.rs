...
#[test]
fn css_flex_row_reverse_keeps_media_link_hit_geometry_after_adjacent_scroll() {
    let image_url = "mem://flex-row-reverse-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![104],
        rgb_pixels: Some(vec![34, 144, 96]),
    };
    let decoded_entry = DecodedImageEntry {
        url: image_url.clone(),
        width: decoded.width,
    ...
    let html = format!(
        r#"
            <html><head><style>
              ...
              <p class="lead">Intro copy before the reversed media card.</p>
              <section class="card">
                <img src="{image_url}" width="24" height="24" alt="">
                <a class="cta" href="/reverse">Open reversed card</a>
              </section>
    ...
    .expect("render row-reverse flex card");
    ...
    let card_background = render
        ...
        .find_map(|command| match command {
            DisplayCommand::Rect {
                ...
                height,
                shade,
            } if *shade == 242 => Some((*x, *y, *width, *height)),
            _ => None,
        })
    ...
    let image_bounds = render
        ...
        .expect("decoded image should render in row-reverse card");
    let (link_command_index, link) = render
        ...
        .find_map(|(index, command)| match command {
            DisplayCommand::Text { x, y, text } | DisplayCommand::StyledText { x, y, text, .. }
                if text.contains("Open reversed") =>
            {
                Some((index, (*x, *y)))
            }
            _ => None,
        })
        .expect("link text should render in row-reverse card");

    assert_eq!(image_bounds.1, card_background.1);
    assert!(
        link.0 < image_bounds.0,
        "row-reverse should place the later source link before the earlier source image, image={image_bounds:?} link={link:?}"
    );
    assert_eq!(
        link.1,
        image_bounds
            .1
            .saturating_add(image_bounds.3.saturating_sub(1) / 2),
        "row-reverse should preserve cross-axis centering beside the image"
    );
    let link_target = render
        .hit_targets
        .get(link_command_index)
        .and_then(|target| target.target_at_column(0))
        .expect("row-reverse visual link command should carry the anchor hit target");
    assert_eq!(
        hit_test_text_target_node(&render, link.0, link.1),
        Some(link_target),
        "visual text hit testing should use the row-reverse link row"
    );
    ...
    let viewport_target =
        hit_test_target_node_in_viewport(&render, second_viewport.viewport, link.0, local_link_y)
            .expect("viewport hit testing should hit the row-reverse visual link row");
    assert_eq!(
        anchor_href_for_node(&page_state.dom, viewport_target),
        anchor_href_for_node(&page_state.dom, link_target),
        "viewport hit testing should resolve the same anchor as the row-reverse visual text row"
    );
    ...
    for viewport_y in [first_viewport.viewport.y, second_viewport.viewport.y] {
        ...
        .expect("rasterize adjacent row-reverse viewport");
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [242, 242, 242, 255]),
            "adjacent scrolled raster should retain the reversed card underlay"
        );
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [34, 144, 96, 255]),
...
#[test]
fn css_flex_column_reverse_keeps_media_link_hit_geometry_after_adjacent_scroll() {
    let image_url = "mem://flex-column-reverse-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![112],
        rgb_pixels: Some(vec![84, 118, 220]),
    };
    let decoded_entry = DecodedImageEntry {
        url: image_url.clone(),
        width: decoded.width,
    ...
    let html = format!(
        r#"
            <html><head><style>
              ...
              <p class="lead">Intro copy before the reversed column card.</p>
              <section class="card">
                <a class="cta" href="/column-reverse">Open column card</a>
                <img src="{image_url}" width="32" height="24" alt="">
              </section>
              <p class="tail">Trailing copy keeps adjacent viewport scroll meaningful.</p>
            </body></html>
            "#
    ...
    .expect("render column-reverse flex card");
    let render = profiled.render;
    ...
    let card_background = render
        ...
        .find_map(|command| match command {
            DisplayCommand::Rect {
                ...
                height,
                shade,
            } if *shade == 243 => Some((*x, *y, *width, *height)),
            _ => None,
        })
        .expect("card background should paint");
    let image_bounds = render
        ...
        .expect("decoded image should render in column-reverse card");
    let (link_command_index, link) = render
        .display_list
        .iter()
        .enumerate()
        .find_map(|(index, command)| match command {
            DisplayCommand::Text { x, y, text } | DisplayCommand::StyledText { x, y, text, .. }
                if text.contains("Open column") =>
            {
                Some((index, (*x, *y)))
            }
            _ => None,
        })
        .expect("link text should render in column-reverse card");

    assert_eq!(image_bounds.1, card_background.1);
    assert!(
        link.1 >= image_bounds.1.saturating_add(image_bounds.3),
        "column-reverse should place the later source image before the earlier source link, image={image_bounds:?} link={link:?}"
    );
    assert_eq!(
        link.0, image_bounds.0,
        "column-reverse should preserve the shared flex column inline origin"
    );
    let link_target = render
        .hit_targets
        .get(link_command_index)
        .and_then(|target| target.target_at_column(0))
        .expect("column-reverse visual link command should carry the anchor hit target");
    assert_eq!(
        hit_test_text_target_node(&render, link.0, link.1),
        Some(link_target),
        "visual text hit testing should use the column-reverse link row"
    );
    ...
    assert!(second_viewport.max_scroll_y > 0);
    let local_link_y = link.1.saturating_sub(second_viewport.viewport.y);
    assert!(local_link_y < second_viewport.viewport.height);
    let viewport_target =
        hit_test_target_node_in_viewport(&render, second_viewport.viewport, link.0, local_link_y)
            .expect("viewport hit testing should hit the column-reverse visual link row");
    assert_eq!(
        anchor_href_for_node(&page_state.dom, viewport_target),
        anchor_href_for_node(&page_state.dom, link_target),
        "viewport hit testing should resolve the same anchor as the column-reverse visual text row"
    );
    ...
    for viewport_y in [first_viewport.viewport.y, second_viewport.viewport.y] {
        ...
        .expect("rasterize adjacent column-reverse viewport");
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [243, 243, 243, 255]),
            "adjacent scrolled raster should retain the reversed column card underlay"
        );
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [84, 118, 220, 255]),
            "adjacent scrolled raster should retain decoded image color"
        );
    }
}
...
fn css_flex_wrap_basis_keeps_mixed_cards_stable_in_scrolled_viewport() {
    ...
    let html = format!(
        r#"
            <html><body>
              <section style="display:flex; flex-wrap:wrap; gap:12px 8px; background-color:rgb(250, 250, 250)">
                <div style="flex:0 0 80px">First card</div>
                <img src="{image_url}" alt="" width="16" height="12" style="flex-basis:80px">
                <div style="flex:0 0 80px">Second body</div>
                <img src="{image_url}" alt="" width="16" height="12" style="flex-basis:80px">
              </section>
              <p>After flex</p>
            </body></html>
    ...
    .expect("render flex-wrap card flow with decoded image");
    ...
    let image_bounds = render
        ...
        .collect::<Vec<_>>();
    assert_eq!(image_bounds.len(), 2);
    assert_eq!(image_bounds[0].1, 0);
    assert!(
        image_bounds[1].1 > image_bounds[0].1,
        "expected flex-wrap to place the third and fourth items on a later row, images={image_bounds:?}"
    );

    let (second_text_x, second_text_y) = render
        .display_list
        .iter()
        .find_map(|command| match command {
            DisplayCommand::Text { x, y, text } | DisplayCommand::StyledText { x, y, text, .. }
                if text.contains("Second body") =>
            {
                Some((*x, *y))
            }
            _ => None,
        })
        .expect("second flex row text command");
    assert_eq!(second_text_y, image_bounds[1].1);
    assert!(
        image_bounds[1].0 >= second_text_x.saturating_add(10),
        "expected flex-basis reservation before the second-row image, text=({second_text_x},{second_text_y}) image={:?}",
        image_bounds[1]
    );
    ...
    let rgba = rasterize_render_rgba(&render, raster_options)
        .expect("rasterize scrolled flex-wrap image/text row");
    let pixel = |x: usize, y: usize| {
        let index = y
            .saturating_mul(rgba.width)
            .saturating_add(x)
    ...
    assert_eq!(
        pixel(image_pixel_x, raster_options.padding_y),
        &[180, 80, 32, 255]
    );
    ...
    assert!(
        glyph_pixels >= 8,
        "expected readable text ink beside decoded image content in the scrolled wrapped flex row"
    );
}
...
#[test]
fn css_align_items_center_keeps_flex_media_text_aligned_after_adjacent_scroll() {
    let image_url = "mem://align-items-center-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![96],
        rgb_pixels: Some(vec![52, 132, 220]),
    };
    let decoded_entry = DecodedImageEntry {
        url: image_url.clone(),
        width: decoded.width,
    ...
    let html = format!(
        r#"
            <html><head><style>
              ...
              <p class="lead">Intro copy before the media nav.</p>
              <nav class="nav">
    ...
    let (page_state, profiled) = render_html_prepared_with_state(
        "mem://align-items-center-flex-row",
        html.as_bytes(),
        BrowserRenderOptions {
            width: 40,
            ..BrowserRenderOptions::default()
    ...
    .expect("render align-items center flex row");
    let render = profiled.render;
    ...
    let nav_background = render
        ...
        .expect("nav background should paint");
    let image_bounds = render
        .display_list
        .iter()
        .find_map(|command| match command {
        ...
        .expect("decoded image should render in flex row");
    let (link_command_index, link) = render
        .display_list
        .iter()
        .enumerate()
        .find_map(|(index, command)| match command {
            DisplayCommand::Text { x, y, text } | DisplayCommand::StyledText { x, y, text, .. }
                if text.contains("Open docs") =>
            {
                Some((index, (*x, *y)))
            }
            _ => None,
        })
        .expect("link text should render beside image");

    assert_eq!(image_bounds.1, nav_background.1);
    assert_eq!(
        link.1,
        image_bounds.1.saturating_add(1),
        "align-items:center should center one-row link text beside the taller image"
    );
    ...
    let link_target = render
        ...
        .get(link_command_index)
        .and_then(|target| target.target_at_column(0))
        .expect("centered visual link command should carry the anchor hit target");
    assert_eq!(
        hit_test_text_target_node(&render, link.0, link.1),
        Some(link_target),
        "visual text hit testing should use the centered link row"
    );
    ...
    let viewport_target =
        hit_test_target_node_in_viewport(&render, second_viewport.viewport, link.0, local_link_y)
            .expect("viewport hit testing should hit the centered visual text row");
    assert_eq!(
        anchor_href_for_node(&page_state.dom, viewport_target),
        anchor_href_for_node(&page_state.dom, link_target),
        "viewport hit testing should resolve the same anchor as the centered visual text row"
    );
    ...
    for viewport_y in [first_viewport.viewport.y, second_viewport.viewport.y] {
        ...
        .expect("rasterize adjacent align-items viewport");
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [246, 246, 246, 255]),
            "adjacent scrolled raster should retain the nav underlay"
        );
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [52, 132, 220, 255]),
            "adjacent scrolled raster should retain decoded image color"
        );
    }
}
...
#[test]
fn css_align_items_flex_end_keeps_flex_media_link_hit_after_adjacent_scroll() {
    let image_url = "mem://align-items-flex-end-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![88],
        rgb_pixels: Some(vec![180, 86, 44]),
    };
    let decoded_entry = DecodedImageEntry {
        url: image_url.clone(),
        width: decoded.width,
    ...
    let html = format!(
        r#"
            <html><head><style>
              ...
              .details {{ background: rgb(232, 232, 232); }}
              .tail {{ height: 96px; }}
            </style></head><body>
              <p class="lead">Intro copy before the product card.</p>
              <section class="card">
                <img src="{image_url}" width="24" height="36" alt="">
                <a class="details" href="/details">Open product details</a>
              </section>
              <p class="tail">Trailing copy keeps the viewport scrollable.</p>
            </body></html>
            "#
    ...
    .expect("render align-items flex-end flex row");
    let render = profiled.render;
    ...
    let card_background = render
        ...
        .find_map(|command| match command {
            DisplayCommand::Rect {
                ...
                height,
                shade,
            } if *shade == 244 => Some((*x, *y, *width, *height)),
            _ => None,
        })
        .expect("card background should paint");
    let image_bounds = render
        ...
        .expect("decoded image should render in flex-end row");
    let (link_command_index, link) = render
        .display_list
        .iter()
        .enumerate()
    ...
    assert_eq!(
        ...
        "align-items:flex-end should bottom-align one-row link text beside the taller image"
    );
    assert!(link.0 > image_bounds.0.saturating_add(image_bounds.2));
    let link_target = render
        .hit_targets
        .get(link_command_index)
        .and_then(|target| target.target_at_column(0))
        .expect("bottom-aligned visual link command should carry the anchor hit target");
    assert_eq!(
        hit_test_text_target_node(&render, link.0, link.1),
        Some(link_target),
        "visual text hit testing should use the bottom-aligned link row"
    );
    ...
    let viewport_target =
        hit_test_target_node_in_viewport(&render, second_viewport.viewport, link.0, local_link_y)
            .expect("viewport hit testing should hit the bottom-aligned visual text row");
    assert_eq!(
        anchor_href_for_node(&page_state.dom, viewport_target),
        anchor_href_for_node(&page_state.dom, link_target),
        "viewport hit testing should resolve the same anchor as the bottom-aligned visual text row"
    );
    ...
    for viewport_y in [first_viewport.viewport.y, second_viewport.viewport.y] {
        ...
        .expect("rasterize adjacent flex-end viewport");
        assert!(
            rgba.pixels
                .chunks_exact(4)
...
#[test]
fn css_justify_content_space_between_nav_keeps_visible_hit_geometry_after_scroll() {
    let render = render_html(
        "mem://justify-content-nav-flow",
        br#"
            <html><head><style>
              .spacer { height: 24px; }
              .tail { height: 64px; }
    ...
    let nav_background = render
        ...
        .find_map(|command| match command {
            DisplayCommand::Rect {
                ...
                height,
                shade,
            } if *shade == 235 && *width == 40 => Some((*x, *y, *width, *height)),
            _ => None,
        })
    ...
    assert_eq!(docs.0, nav_background.0);
    assert_eq!(install.0, nav_background.0 + 16);
    assert_eq!(learn.0, nav_background.0 + 35);
    ...
    let rgba =
        rasterize_render_rgba(&render, raster_options).expect("rasterize justified nav viewport");
    let pixel = |x: usize, y: usize| {
        let index = y
            .saturating_mul(rgba.width)
            .saturating_add(x)
    ...
    let nav_pixel_x = raster_options.padding_x.saturating_add(
        nav_background
            .0
            .saturating_add(8)
            .saturating_mul(raster_options.cell_width),
    ...
    assert_eq!(pixel(nav_pixel_x, nav_pixel_y), &[235, 235, 235, 255]);
}
...
#[test]
fn overflow_block_clip_keeps_adjacent_scroll_slices_and_hit_geometry_stable() {
    let image_url = "mem://overflow-block-clip-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![96],
    ...
    let html = format!(
        r#"
            <html><head><style>
              ...
              <p class="lead">Intro copy before the clipped card.</p>
              <section class="card">
                <img src="{image_url}" width="24" height="12" alt="">
                <a href="/visible">Visible card link</a>
                <div style="height:48px"></div>
                <a href="/hidden">Hidden overflow link</a>
              </section>
              <p class="tail">Trailing copy keeps the viewport scrollable.</p>
            </body></html>
            "#
    ...
    .expect("render overflow-block clip fixture");
    ...
    let image_bounds = render
        ...
        .expect("decoded image should render inside overflow-block card");
    let visible_link = render
        .display_list
        .iter()
        .find_map(|command| match command {
            DisplayCommand::Text { x, y, text } | DisplayCommand::StyledText { x, y, text, .. }
                if text.contains("Visible card") =>
            {
                Some((*x, *y))
            }
            _ => None,
        })
        .expect("visible link should remain inside overflow-block clip");
    assert!(
        !render.display_list.iter().any(|command| matches!(
            command,
            DisplayCommand::Text { text, .. } | DisplayCommand::StyledText { text, .. }
                if text.contains("Hidden overflow")
        )),
        "overflow-block: clip should cull text below the block clip"
    );
    assert_eq!(card_background.0, 0);
    assert!(image_bounds.1 >= card_background.1);
    assert!(visible_link.1 > image_bounds.1);

    let visible_target = hit_test_target_node(&render, visible_link.0, visible_link.1)
        .expect("visible clipped link should hit");
    let first_viewport = browser_document_viewport(
        &render,
        BrowserViewportState {
            x: 0,
    ...
    let local_link_y = visible_link.1.saturating_sub(second_viewport.viewport.y);
    assert_eq!(
        ...
        Some(visible_target),
        "viewport hit testing should stay aligned with visible overflow-block content"
    );
    ...
    for viewport_y in [first_viewport.viewport.y, second_viewport.viewport.y] {
        ...
        .expect("rasterize adjacent overflow-block viewport");
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [244, 244, 244, 255]),
        ...
        assert!(
            rgba.pixels
                .chunks_exact(4)
                .any(|pixel| pixel == [48, 132, 220, 255]),
            "adjacent scrolled raster should retain decoded image color"
        );
    }
}
...
#[test]
fn viewport_hit_testing_prefers_topmost_fixed_background_over_scrolled_text() {
    let image_url = "mem://fixed-hit-topmost-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![120],
    ...
    let html = format!(
        r#"
            <html><body>
              <a href="/fixed" style="display:block; position:fixed; top:0; width:192px; height:24px; background:rgb(245,245,245); color:rgb(0,0,0)">Nav</a>
    ...
    .expect("render fixed topmost hit fixture");
    let render = profiled.render;
    ...
    let fixed_background_index = render
        ...
        .expect("fixed header background should paint");
    ...
    let click_x = underlying_text.0.saturating_add(12);
    assert!(
        fixed_bounds.contains(click_x, viewport.viewport.y),
        "fixed background should cover the same viewport coordinate as underlying text"
    );
    assert!(
    ...
    let viewport_target = hit_test_target_node_in_viewport(&render, viewport.viewport, click_x, 0)
        .expect("viewport hit should resolve the painted topmost target");
    assert_eq!(
        anchor_href_for_node(&page_state.dom, viewport_target).as_deref(),
        Some("/fixed"),
        "viewport hit testing should prefer the painted fixed background over underlying text"
    );
    ...
    let rgba =
        rasterize_render_rgba(&render, raster_options).expect("rasterize fixed topmost hit slice");
    let pixel_x = raster_options
        .padding_x
...
#[test]
fn viewport_normal_flow_overlay_blocks_link_only_while_visible_after_adjacent_scroll() {
    let render = BrowserRender {
        source: "mem://normal-overlay-scroll-hit".to_owned(),
        title: String::new(),
        viewport_width: 10,
        dom_node_count: 0,
        css_rule_count: 0,
        ...
        display_list: vec![
            ...
            DisplayCommand::Text {
                x: 0,
                y: 5,
                text: "Body row after overlay".to_owned(),
            },
            DisplayCommand::Text {
                x: 0,
                y: 7,
    ...
    assert_eq!(
        hit_test_target_node_in_viewport(&render, first.viewport, 2, 0),
        None,
        "later normal-flow overlay should block linked text under its painted cells"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, first.viewport, 6, 0),
        Some(77),
        "uncovered linked text should remain exactly hittable in the same viewport slice"
    );
    ...
    assert_eq!(
        hit_test_target_node_in_viewport(&render, second.viewport, 6, 0),
        None,
        "the linked row should stop resolving once it has scrolled out of the viewport"
    );
    ...
    let rgba = rasterize_render_rgba(&render, options).expect("rasterize normal overlay slice");
    let pixel = |x: usize, y: usize| {
        let index = y
            .saturating_mul(rgba.width)
            .saturating_add(x)
    ...
    assert_eq!(
        pixel(options.padding_x, options.padding_y),
        &[220, 64, 64, 255],
        "raster should show the later normal-flow overlay at the blocked hit cell"
    );
    let link_cell_start = options
        .padding_x
        .saturating_add(6usize.saturating_mul(options.cell_width));
    ...
    let link_row_end = options
        ...
        .saturating_add(options.cell_height)
        .min(rgba.height);
    let uncovered_link_has_ink = (options.padding_y..link_row_end)
        .any(|y| (link_cell_start..link_cell_end).any(|x| pixel(x, y) == &[0, 0, 0, 255]));
    assert!(
        uncovered_link_has_ink,
        "raster should retain visible linked text outside the overlay"
    );
}
...
#[test]
fn repeated_small_scroll_slices_drop_stale_link_hits_after_visible_rows_move() {
    let image_url = "mem://small-slice-stability-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![88],
        rgb_pixels: Some(vec![42, 132, 216]),
    };
    let render = BrowserRender {
        source: "mem://small-slice-stability".to_owned(),
        title: String::new(),
        viewport_width: 10,
        dom_node_count: 0,
        css_rule_count: 0,
    ...
    let frames =
        browser_viewport_frame_sequence(&render, start, &[(0, 1), (0, 1), (0, 1)], options)
            .expect("render repeated small scroll slices");
    assert_eq!(frames.len(), 3);

    let expected_rows = [
        vec![(1, "Image", 0), (2, "StyledText", 1), (3, "ColorRect", 2)],
        vec![(2, "StyledText", 0), (3, "ColorRect", 1), (4, "Text", 2)],
        vec![(3, "ColorRect", 0), (4, "Text", 1), (5, "Text", 2)],
    ];
    for (index, frame) in frames.iter().enumerate() {
        let expected_y = index + 2;
        assert_eq!(frame.report.viewport.viewport.y, expected_y);
        assert_eq!(frame.report.viewport.scroll_delta_y, 1);
        assert_eq!(frame.report.frame.raster_viewport_y, Some(expected_y));
        assert_eq!(
            frame
                ...
                .collect::<Vec<_>>(),
            expected_rows[index],
            "scroll slice {index} should advance visible command rows monotonically"
        );
        assert!(
            frame
                .report
                ...
                .iter()
                .any(|region| region.viewport_y == start.height - 1 && region.viewport_height == 1),
            "scroll slice {index} should dirty the newly exposed bottom row"
        );
    }
    ...
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[0].report.viewport.viewport, 1, 0),
        Some(41),
        "image target should be hittable while painted at the top row"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[0].report.viewport.viewport, 1, 1),
        Some(77),
        "link target should be hittable while painted below the image"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[1].report.viewport.viewport, 1, 0),
        Some(77),
        "link target should move to the top row after one more scroll"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[1].report.viewport.viewport, 1, 1),
        None,
        "image target should not leak into the background row"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[2].report.viewport.viewport, 1, 0),
        None,
        "link target should not remain hittable after it scrolls out"
    );
    ...
    assert_eq!(
        pixel(&frames[0], target_x, options.padding_y),
        [42, 132, 216, 255],
        "first small-scroll slice should show the decoded image at the top row"
    );
    let link_has_top_ink = |frame: &BrowserViewportFrame| {
        (options.padding_y
            ..options
                .padding_y
                .saturating_add(options.cell_height)
                .min(frame.raster.height))
            .any(|y| {
                (target_x..target_x.saturating_add(options.cell_width))
                    .any(|x| pixel(frame, x, y) == [0, 0, 0, 255])
            })
    };
    assert!(
        link_has_top_ink(&frames[1]),
        "second small-scroll slice should paint the link at the top row"
    );
    assert_eq!(
        pixel(&frames[2], options.padding_x, options.padding_y),
        [236, 244, 250, 255],
        "third small-scroll slice should replace the link with the background row"
    );
}
...
#[test]
fn continuous_small_scroll_state_keeps_mixed_rows_and_clamped_hits_stable() {
    let image_url = "mem://continuous-scroll-state-image".to_owned();
    let decoded = DecodedImage {
        width: 1,
        height: 1,
        pixels: vec![92],
        rgb_pixels: Some(vec![44, 136, 224]),
    };
    let render = BrowserRender {
        source: "mem://continuous-scroll-state".to_owned(),
        title: String::new(),
        viewport_width: 14,
        dom_node_count: 0,
        css_rule_count: 0,
    ...
    let frames =
        browser_viewport_frame_sequence(&render, start, &[(0, 1), (0, 1), (0, 1), (0, 1)], options)
            .expect("render continuous small scroll state frames");
    ...
    assert_eq!(
        frames
            ...
            .collect::<Vec<_>>(),
        vec![4, 5, 6, 6],
        "viewport_y should advance monotonically and then clamp at the bottom"
    );
    assert_eq!(
        frames
            .iter()
    ...
    assert_eq!(
        ...
        "first scroll step should expose the mixed image/control/link row and following row"
    );
    assert_eq!(
        frames[1]
            .report
            ...
            .collect::<Vec<_>>(),
        vec![(4, "ColorRect", 0), (5, "Text", 1)],
        "next scroll step should drop the mixed row without duplicating it"
    );
    assert!(
        frames[3].report.viewport.invalidated_regions.is_empty(),
        "repeated scroll after clamping should not invent dirty document rows"
    );
    assert!(
        frames[3].report.dirty_pixel_regions.is_empty(),
        "repeated scroll after clamping should not invent dirty raster rows"
    );
    assert_eq!(
        frames[3].report.reused_pixel_area,
        frames[3]
            .report
            .frame_width
            .saturating_mul(frames[3].report.frame_height),
        "clamped no-op frame should report full raster reuse"
    );

    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[0].report.viewport.viewport, 1, 0),
        Some(41),
        "decoded image hit should align with the visible mixed row"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[0].report.viewport.viewport, 4, 0),
        Some(12),
        "control hit should align with the visible mixed row"
    );
    assert_eq!(
        hit_test_target_node_in_viewport(&render, frames[0].report.viewport.viewport, 9, 0),
        Some(77),
        "link hit should align with the visible mixed row"
    );
    for (index, frame) in frames.iter().enumerate().skip(1) {
        assert_eq!(
            hit_test_target_node_in_viewport(&render, frame.report.viewport.viewport, 1, 0),
            None,
            "frame {index} should not keep stale image hits after the mixed row scrolls out"
        );
        assert_eq!(
            hit_test_target_node_in_viewport(&render, frame.report.viewport.viewport, 4, 0),
            None,
            "frame {index} should not keep stale control hits after the mixed row scrolls out"
        );
        assert_eq!(
            hit_test_target_node_in_viewport(&render, frame.report.viewport.viewport, 9, 0),
            None,
            "frame {index} should not keep stale link hits after the mixed row scrolls out"
        );
    }
    ...
    assert_eq!(
        pixel(&frames[0], image_x, options.padding_y),
        [44, 136, 224, 255],
        "first scroll frame should paint the decoded image in the visible row"
    );
    assert_ne!(
        pixel(&frames[1], image_x, options.padding_y),
        [44, 136, 224, 255],
        "next scroll frame should not keep stale image pixels in the top row"
    );
    assert_eq!(
